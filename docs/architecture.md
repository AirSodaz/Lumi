...
当前 V1 状态：Tauri 配置启用窗口透明与 `windowEffects` 的 `mica`/`acrylic` intent，但 `settings_get_material_state` 仍返回 fallback probing 文案。Settings -> Appearance 必须诚实展示 fallback 原因，直到 Rust/platform 层实现真实能力探测。
...
- Search：V1 搜索入口。
- Settings：Servers、Player、Appearance、Logs。
