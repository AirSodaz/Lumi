...
- **视觉目标**：参考最新 Apple TV 的沉浸式媒体浏览、聚焦态和大圆角语言，同时尽量使用系统原生材质。Windows shell 优先 Mica，macOS shell 优先 Sidebar material；CSS glass 只补充内容层。
...
- Windows 11 的 Mica 真实窗口效果、播放器窗口、全屏和安装包。
- 当前 macOS 的 Sidebar material 可用范围、播放器窗口和打包限制。
